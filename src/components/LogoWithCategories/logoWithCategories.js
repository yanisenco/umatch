...
const LogoWithCategories = () => {
  ...
  // ];
    const categories = [
    { title: t('actorSection.artists'), image: artists, height: "md:h-[250px]", width: "w-[142px]" },
    { title: t('actorSection.influencers'), image: influencers, height: "md:h-[450px]", width: "w-[255px]" },
    { title: t('actorSection.brands'), image: brands, height: "md:h-[400px]", width: "w-[225px]" },
    { title: t('actorSection.eSport'), image: esport, height: "md:h-[600px]", width: "w-[310px]" },
    { title: t('actorSection.actors'), image: actors, height: "md:h-[450px]", width: "w-[255px]" },
    { title: t('actorSection.labels'), image: labels, height: "md:h-[200px]", width: "w-[113px]" }
  ];
  ...
  return (
    <div className="flex flex-col md:flex-row justify-center items-center gap-3 md:gap-6 py-10">
      {categories.map((category, index) => (
        <div
          key={index}
          className={`relative ${category.color} bg-dark-blue flex flex-col items-center justify-center p-4 rounded-full h-[70px] ${category.height} ${category.width} md:w-[130px]`}
        >
